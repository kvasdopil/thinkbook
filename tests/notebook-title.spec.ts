...
test.describe('Notebook Title', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[aria-label="Show notebook panel"]', { timeout: 5000 });
  });
  ...
  test('clicking anywhere on title area focuses the input', async ({ page }) => {
    // Create a new file
    await page.click('button[aria-label="Create new notebook"]');
    
    // Click on the title area (not directly on input)
    const titleArea = page.locator('[aria-label="Click to edit notebook title"]');
  ...
  test('settings button opens settings modal', async ({ page }) => {
    ...
    const settingsButton = page.locator('[aria-label="Notebook header"] button[aria-label="Open settings"]');
  ...
  test('header has proper ARIA roles and labels', async ({ page }) => {
    ...
    const header = page.locator('[role="banner"][aria-label="Notebook header"]');

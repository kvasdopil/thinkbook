...
test.describe('AI Chat', () => {
  ...
  test('displays AI chat interface when API key is configured', async ({ page }) => {
    ...
    await expect(page.locator('textarea[placeholder="Ask the AI assistant..."]')).toBeVisible();
    await expect(page.locator('button:has-text("Send")')).toBeVisible();
    await expect(page.locator('text=Start a conversation with the AI assistant')).toBeVisible();
  });

  test('displays configuration message when API key is not available', async ({ page }) => {
    ...
    await expect(page.locator('text=AI Chat is not available')).toBeVisible();
    await expect(page.locator('text=Please configure your Gemini API key in settings to use AI chat.')).toBeVisible();
  });
  ...
  test('allows typing in the chat input', async ({ page }) => {
    ...
    const chatInput = page.locator('textarea[placeholder="Ask the AI assistant..."]');
  ...
  test('chat history is positioned above code cell placeholder', async ({ page }) => {
    // Configure API key first via the auto-opened modal
    await expect(page.locator('[role="dialog"]')).toBeVisible();
    await page.fill('#gemini-api-key', 'test-api-key');
    await page.fill('#snowflake-access-token', 'test-access-token');

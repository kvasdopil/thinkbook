...
import { useNotebookFilesV2 } from './hooks/useNotebookFilesV2';
import { useNotebookChatV2 } from './hooks/useNotebookChatV2';
import type { SettingsConfig } from './components/SettingsModal';
import type { NotebookFile } from './types/notebook';
import { storage } from './utils/storage';

// Simplified AiChat component that uses the new hook
function AiChatV2({ currentNotebook }: { currentNotebook?: NotebookFile | null }) {
  const chat = useNotebookChatV2({ currentNotebook });
  // ... rest of AiChat implementation using the chat hook
  // This would be identical to the current AiChat but using useNotebookChatV2
  return <AiChat currentNotebook={currentNotebook} />;
}

function AppV2() {
  const [isSettingsOpen, setIsSettingsOpen] = useState(false);
  const { activeFile, createFile, setActiveFile } = useNotebookFilesV2();
    ...
    <div className="flex flex-col h-screen bg-gray-50 font-sans">
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col">
          <AiChatV2 currentNotebook={activeFile} />
        </div>
      </main>
